...
export class TodosService {
   ...
   constructor(@InjectModel('todo') private readonly todoModel: Model<todo>){}
    ...
    async createToDo(title: string, description: string, completed: boolean, isDeleted: boolean, isArchived: boolean){
    ...
    async getSingleList(id: string){
      const listItem = await this.todoModel.findById(id).exec();

      return listItem;
    }

    async updateList(id: string, title: string, description: string,  completed: boolean, isDeleted: boolean, isArchived: boolean){
